      ...
      if (type === 'HBAR' && req.currentUser?.id) {
        const newCampaign = await prisma.campaign_twittercard.create({
          data: {
            name,
            tweet_text,
            comment_reward: convertToTinyHbar(comment_reward as number),
            like_reward: convertToTinyHbar(like_reward as number),
            retweet_reward: convertToTinyHbar(retweet_reward as number),
            quote_reward: convertToTinyHbar(quote_reward as number),
            campaign_budget: convertToTinyHbar(campaign_budget as number),
