...
const calculateMaxActivityReward = (engagers: number, budget: number) => budget / engagers / 4;

const getRewardsValues = (reward: number, type: CampaignTypes, tokenId?: string, decimals?: number) => {
 ...
 */
export const draftCampaign = async (campaignBody: DraftCampaignBody, userId: number | bigint) => {

    const { name, tweet_text, expected_engaged_users, campaign_budget, type, media, fungible_token_id } = campaignBody;
    const contract_id = generateRandomString(20); // Generate a random contract ID
    const maxActivityReward = calculateMaxActivityReward(expected_engaged_users, Number(campaign_budget)); // Calculate the maximum reward per activity
   
    const prisma = await PrismaClientManager.getInstance(); // Get an instance of Prisma client

    try {
        // Fetch token data if the campaign type is FUNGIBLE
        const tokenData = await prisma.whiteListedTokens.findUnique({
            where: { token_id: fungible_token_id?.toString() },
        });

        // Calculate reward values for different activities and the total campaign budget
        const rewardValues = await Promise.all([
            getRewardsValues(maxActivityReward, type, fungible_token_id, tokenData?.decimals?.toNumber()),
            getRewardsValues(maxActivityReward, type, fungible_token_id, tokenData?.decimals?.toNumber()),
            getRewardsValues(maxActivityReward, type, fungible_token_id, tokenData?.decimals?.toNumber()),
            getRewardsValues(maxActivityReward, type, fungible_token_id, tokenData?.decimals?.toNumber()),
            getRewardsValues(campaign_budget, type, fungible_token_id, tokenData?.decimals?.toNumber())
        ]);

        const [comment_reward, retweet_reward, like_reward, quote_reward, campaign_budget_value] = rewardValues;
        ...
        const newCampaign = await prisma.campaign_twittercard.create({ data: campaignData });
        const sanitizedData = rmKeyFrmData(newCampaign, ['last_reply_checkedAt', 'last_thread_tweet_id', 'contract_id']); // Remove sensitive keys from the data
